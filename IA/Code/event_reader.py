"""
Lecteur d'événements Windows - CORRECTIF FICHIER VERROUILLÉ
Fichier : event_reader.py - VERSION FINALE
✅ FORCE la copie temporaire si fichier verrouillé (erreur 32)
✅ Logs 100% silencieux en surveillance
✅ Gestion intelligente des copies
"""
...
class EventReader:
    ...
    def cleanup_old_temp_files(self):
        """
        🔥 Nettoie UNIQUEMENT les copies des AUTRES processus
        Ne touche PAS à la copie du processus actuel
        """
        try:
            ...
            for file in os.listdir(self.temp_dir):
                ...
                # Extraire le PID du nom de fichier
                try:
                    file_pid = int(file.replace('temp_', '').replace('.evtx', ''))
                    
                    # Ne supprimer QUE si c'est un autre processus
                    if file_pid != self.current_pid:
                        ...
                        try:
                            ...
                            pass  # Fichier en cours d'utilisation
                except:
                    pass
    ...
    def create_temp_copy(self, silent=False):
        """Crée une copie temporaire UNIQUE pour ce processus"""
        try:
            os.makedirs(self.temp_dir, exist_ok=True)
            
            # Nom unique basé sur le PID
            temp_file = os.path.join(self.temp_dir, f"temp_{self.current_pid}.evtx")
            
            # Si la copie existe déjà, la supprimer
    ...
    def check_availability(self):
        ...
        # Nettoyer les vieilles copies au démarrage
        ...
        try:
            ...
            # 🔥 DÉTECTION ERREUR 32 (FICHIER VERROUILLÉ)
    ...
    def get_working_file(self):
        ...
        # 🔥 SI MODE COPIE FORCÉ, TOUJOURS utiliser la copie
    ...
    def read_events(self, since_time=None, since_record=None, silent=False):
        ...
        if not working_file or not os.path.exists(working_file):
            # Fichier introuvable → recréer copie
    ...
    def cleanup(self):
        """Nettoyage manuel (à appeler à la fermeture)"""
        if self.temp_log_file and os.path.exists(self.temp_log_file):
            try:
                os.remove(self.temp_log_file)
                self.temp_log_file = None
